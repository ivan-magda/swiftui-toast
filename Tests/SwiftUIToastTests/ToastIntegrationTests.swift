...
struct ToastIntegrationTests {
    ...
    func toastLifecycle() async throws {
        // Given
        ...
        // Setup observation with confirmation
        try await confirmation { confirm in
            let observer = ToastObserver(
                toastManager: toastManager,
                onChangeCount: 2
            ) { currentID in
                toastIDChanges.append(currentID)
            } onComplete: {
                confirm()
            }
    ...
    func toastModifierBindingUpdates() {
        // Given
        let toastManager = ToastManager()
...
struct ToastSwiftUIIntegrationTests {
    ...
    func viewModifierWithFalseBinding() {
        let view = Text("Test View")
            .toast(
